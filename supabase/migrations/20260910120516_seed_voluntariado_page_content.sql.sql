/*
# Seed Voluntariado page content

1. Updates the cms_pages row for slug='voluntariado':
   - banner_image now points to the local volunteer image
   - subtitle updated to an editable intro line
2. Inserts three cms_blocks of type 'text' that serve as the
   three selectable tabs on the Voluntariado page:
   - "Hazte voluntario"
   - "Oficina del voluntariado"
   - "Cómo participar"
3. No security changes — all tables already have RLS + public read / auth write policies.
*/
...
0, true
FROM cms_pages WHERE slug = 'voluntariado'
