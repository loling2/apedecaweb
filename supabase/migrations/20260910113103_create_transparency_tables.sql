/*
# Tablas de Transparencia gestionables desde el CMS

1. Nuevas tablas
- `cms_transparency_sections`: cada apartado de la pestaña Transparencia
- `cms_transparency_items`: líneas/acordeones dentro de cada sección
- `cms_transparency_docs`: documentos PDF asociados a cada item
...
CREATE TABLE IF NOT EXISTS cms_transparency_sections (
...
CREATE POLICY "public_read_transparency_sections" ON cms_transparency_sections FOR SELECT
  TO anon, authenticated USING (true);

DROP POLICY IF EXISTS "auth_insert_transparency_sections" ON cms_transparency_sections;
CREATE POLICY "auth_insert_transparency_sections" ON cms_transparency_sections FOR INSERT
  TO authenticated WITH CHECK (true);

DROP POLICY IF EXISTS "auth_update_transparency_sections" ON cms_transparency_sections;
CREATE POLICY "auth_update_transparency_sections" ON cms_transparency_sections FOR UPDATE
  TO authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "auth_delete_transparency_sections" ON cms_transparency_sections;
CREATE POLICY "auth_delete_transparency_sections" ON cms_transparency_sections FOR DELETE
  TO authenticated USING (true);

CREATE TABLE IF NOT EXISTS cms_transparency_items (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE POLICY "public_read_transparency_items" ON cms_transparency_items FOR SELECT
  TO anon, authenticated USING (true);

DROP POLICY IF EXISTS "auth_insert_transparency_items" ON cms_transparency_items;
CREATE POLICY "auth_insert_transparency_items" ON cms_transparency_items FOR INSERT
  TO authenticated WITH CHECK (true);

DROP POLICY IF EXISTS "auth_update_transparency_items" ON cms_transparency_items;
CREATE POLICY "auth_update_transparency_items" ON cms_transparency_items FOR UPDATE
  TO authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "auth_delete_transparency_items" ON cms_transparency_items;
CREATE POLICY "auth_delete_transparency_items" ON cms_transparency_items FOR DELETE
  TO authenticated USING (true);

CREATE TABLE IF NOT EXISTS cms_transparency_docs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE POLICY "public_read_transparency_docs" ON cms_transparency_docs FOR SELECT
  TO anon, authenticated USING (true);

DROP POLICY IF EXISTS "auth_insert_transparency_docs" ON cms_transparency_docs;
CREATE POLICY "auth_insert_transparency_docs" ON cms_transparency_docs FOR INSERT
  TO authenticated WITH CHECK (true);

DROP POLICY IF EXISTS "auth_update_transparency_docs" ON cms_transparency_docs;
CREATE POLICY "auth_update_transparency_docs" ON cms_transparency_docs FOR UPDATE
  TO authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "auth_delete_transparency_docs" ON cms_transparency_docs;
CREATE POLICY "auth_delete_transparency_docs" ON cms_transparency_docs FOR DELETE
  TO authenticated USING (true);

CREATE INDEX IF NOT EXISTS idx_transparency_items_section ON cms_transparency_items(section_id);
CREATE INDEX IF NOT EXISTS idx_transparency_docs_item ON cms_transparency_docs(item_id);

-- ==================== DATOS INICIALES ====================
INSERT INTO cms_transparency_sections (label, icon_name, tone, sort_order, meta) VALUES
...
INSERT INTO cms_transparency_items (section_id, title, sort_order, is_open_by_default, download_label)
SELECT id, 'Información General (revisado 13/11/2025)', 0, false, 'Descargar información institucional'
FROM cms_transparency_sections WHERE label = 'Información Institucional'
ON CONFLICT DO NOTHING;
...
FROM cms_transparency_sections WHERE label = 'Organizativa'
ON CONFLICT DO NOTHING;
...
INSERT INTO cms_transparency_items (section_id, title, body, sort_order, is_open_by_default, download_label)
...
-- Contratos
INSERT INTO cms_transparency_items (section_id, title, body, sort_order, is_open_by_default)
...
0, true
FROM cms_transparency_sections WHERE label = 'Contratos'
ON CONFLICT DO NOTHING;

-- Convenios
INSERT INTO cms_transparency_items (section_id, title, sort_order, is_open_by_default, download_label)
SELECT id, 'Convenio 2023', 0, false, 'Descargar documentos - Convenio 2023'
FROM cms_transparency_sections WHERE label = 'Convenios y Encomiendas de gestión'
ON CONFLICT DO NOTHING;
...
-- Ayudas y Subvenciones (2019-2025)
INSERT INTO cms_transparency_items (section_id, title, sort_order, is_open_by_default, download_label)
...
FROM cms_transparency_sections WHERE label = 'Ayudas y Subvenciones'
ON CONFLICT DO NOTHING;
...
FROM cms_transparency_sections WHERE label = 'Gestión de calidad'
ON CONFLICT DO NOTHING;
...
FROM cms_transparency_items WHERE title = 'Buen Gobierno y Transparencia'
ON CONFLICT DO NOTHING;
...
FROM cms_transparency_sections WHERE label = 'Evaluación de transparencia'
ON CONFLICT DO NOTHING;
...
-- Memorias Anuales (2016-2024)
INSERT INTO cms_transparency_items (section_id, title, sort_order, is_open_by_default, download_label)
SELECT id, 'Memoria 2024', 0, false, 'Descargar memoria 2024'
FROM cms_transparency_sections WHERE label = 'Memorias Anuales'
ON CONFLICT DO NOTHING;
